#!/usr/bin/env python3
"""Save untouched HFLM raw scores for one frozen Gate G cell/shard, without gold."""
import argparse
...
def main():
    ...
    from tflt.loopscope.phase8_gate_g_accuracy import validate_manifest, checked_basis as checked_f_basis
    validate_manifest(manifest,pool,a.pool)
    if a.scope != manifest['scope']: raise ValueError('Gate G manifest scope mismatch')
            ...
            if runtime:
                ...
                if runtime:
                    ...
                    if set(c['t'] for c in runtime.calls)!=set(range(cell['k'])) or len(set(counts.values()))!=1 or any(c['applied']!=(c['t'] >= 1) for c in runtime.calls):
                        raise ValueError('Gate G requires equal per-t callbacks and intervention only t>=1')
