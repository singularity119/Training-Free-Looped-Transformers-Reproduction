#!/usr/bin/env python3
"""Save untouched HFLM raw scores for one frozen Gate F cell/shard, without gold."""
import argparse
...
import sys
import time
from tflt.loopscope.phase8_accuracy import select_rows,write_json
...
def main():
    ...
    from tflt.loopscope.phase8_gate_f_accuracy import validate_manifest, checked_basis as checked_f_basis
    validate_manifest(manifest,pool,a.pool)
    if a.scope != manifest['scope']: raise ValueError('Gate F manifest scope mismatch')
    ...
    basis=checked_f_basis(cell,a.scope) if cell['arm']=='Spectral' else None
    ...
    if runtime:
        positions['callback_by_t']={str(t):0 for t in range(cell['k'])}
        positions['applied_by_t']={str(t):0 for t in range(cell['k'])}
    ...
    with manager,(a.run_root/'scores.jsonl').open('x') as out:
        for i,row in enumerate(rows):
            ...
            if runtime:
                ...
                if runtime:
                    counts={t:sum(c['t']==t for c in runtime.calls) for t in range(cell['k'])}
                    if set(c['t'] for c in runtime.calls)!=set(range(cell['k'])) or len(set(counts.values()))!=1 or any(c['applied']!=(c['t'] >= 1) for c in runtime.calls):
                        raise ValueError('Gate F requires equal per-t callbacks and intervention only t>=1')
