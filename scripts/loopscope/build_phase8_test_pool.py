#!/usr/bin/env python3
"""Export the complete canonical test14042 using the offline safe renderer."""
...
from tflt.loopscope.phase8_test_pool import (
    ...
    build_test_pool, write_test_pool,
)
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({"dataset": DATASET_REPO, "revision": DATASET_REVISION,
                          "seed": SEED, "test_identity_count": 14042,
                          "splits": ["test", "dev"], "offline": True,
                          "model_forward": False, "status": "FORMAL_TEST_GOLD_FREE",
    ...
    bundle = build_test_pool(config, args.cache_dir)
    write_test_pool(bundle, args.output_dir)
    print(json.dumps({"output_dir": args.output_dir, "test_count": len(bundle["rows"])}))
