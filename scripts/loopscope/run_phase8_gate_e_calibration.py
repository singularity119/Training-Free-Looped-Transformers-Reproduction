#!/usr/bin/env python3
"""Gate E fixed 4B K3 unmodified Euler answer-residual acquisition; no target outcomes."""
import argparse
...
def main(argv=None):
    ...
    p.add_argument('--cell-index',type=int,choices=range(2),required=True)
    ...
    window,k=spec['windows'][args.cell_index]['layers'],3
    if args.dry_run:
    ...
    from tflt.loopscope.phase8_runtime import AnswerResidualCollector,Phase8Runtime,fit_basis,save_basis,load_basis
    ...
    basis=load_basis(args.run_root/'basis.json',metadata)
