"""Outcome-free Gate G closure tests using the canonical full panel shape."""
import json
...
from tflt.loopscope.phase8_gate_g_accuracy import SCHEMA, panel
...
class GateGVerifyTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        for cell in cells:
            ...
            metadata = dict(cell=cell, pool=str(cls.pool), manifest=str(cls.manifest),
                source_commit='fixed-g-producer', scope='FORMAL_TEST', start=0, end=14042)
            ...
            write_json(cell_root / 'env.json', dict(source_commit='fixed-g-producer',
    ...
    def test_missing_cell_cannot_close_full_panel(self):
        with self.assertRaisesRegex(ValueError, 'all 9 cells'):
    ...
    def test_fitting_at_t0_does_not_authorize_t0_intervention(self):
        path = self.roots[2] / 'summary.json'
        ...
        try:
            ...
            with self.assertRaisesRegex(ValueError, 'wrong intervention timing'):
                self.check([self.roots[2]], full_panel=False)
        finally:
