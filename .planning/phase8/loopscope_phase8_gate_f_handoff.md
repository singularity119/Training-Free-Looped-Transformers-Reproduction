...
## Concurrent source isolation supplement
E confirms its launchers hardcode the shared HPC source and require producer a818dbb. F must not fast-forward or modify that shared source while E owns it. F is explicitly authorized to create a fixed snapshot of its committed code in W/staging/phase8-gate-f-<timestamp>-source-a<N>, using its launcher with an explicit source path and the existing venv. Record readable Git revision and path; debug and formal runs use the same immutable snapshot. This replaces shared-source fast-forward permission during concurrent E execution. No science or budget change.
