...
## Shared connectivity recovery owned by Gate E executor
E reports VPN utun7 at 10.21.0.217 while campus DNS 10.90.63.2/.3 routes through Clash utun4/198.18.0.1; job12687405 state remains unknown. Planning grants E executor01a08031-a85e-7101-b150-60916abdc4ac the smallest reversible local route repair: read current interfaces/routes first, temporarily route those two campus DNS host addresses through the actually active EasyConnect interface, using sudo -n only if already available without credentials. Once campus DNS resolves HPC, the same permission covers only the resolved approved HPC host address if its route is demonstrably wrong. Record original routes and commands; keep EasyConnect and Clash enabled. No default-route/subnet-wide edits, hosts-file changes, credentials, service resets, persistent configuration, or other destinations. If privilege is unavailable, stop that mutation and report the exact limitation; continue safe diagnostics without requesting passwords. Strict SSH verification remains enabled. Restore the affected original route if the repair fails or the VPN interface becomes invalid. E alone owns this shared repair; F must not concurrently change networking. Connectivity restoration does not imply job success: inspect existing jobs before continuation, never resubmit unknown-state work because of disconnection.
