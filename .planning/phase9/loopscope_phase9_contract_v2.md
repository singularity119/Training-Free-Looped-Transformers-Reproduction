...
用户明确取消4B/12:15及1.7B/6:9，目标是在历史上更有希望的loop窗口检验方法效果潜力。两窗口从本阶段诊断、新实验和最终逻辑面板全部移除；第八阶段全部代码、配置、basis和历史结果保持不变。v1保留历史，当前v2优先。此次缩减发生在第九阶段实际模型实验及outcome之前，不根据第九阶段结果筛选。

该设计依据历史accuracy选窗，属于有条件的效果潜力验证，不声称证明严格效果上界、最优窗口或跨所有窗口的普遍收益。保留窗口的K2/3/4全部运行，即使某个K的历史acc低于Native也不再逐K筛选。

## 模型、数据与计算

- Qwen/Qwen3-4B-Base revision 906bfd4b4dc7f14ee4320094d8b41684abff8539，BF16/cache first，零基inclusive 13:16、15:18，边界B13→B17、B15→B19。
- Qwen/Qwen3-1.7B-Base revision ea980cb0a6c2ae4b936e82123acc929f1cec04c1，FP16/cache last，12:15，边界B12→B16。tokenizer同对应model revision。
- 全部三窗口K=2,3,4；K包含t0；alpha1、步长1/K、总时长1、beta0、block damped_euler、batch1、decode bypass、eval/no_grad、不训练、不量化、不生成CoT。
...
最终逻辑面板：三窗口×K2/3/4×[Loop,Shared-t1,Shared-t0,Online-t0,Matched-norm] + 两个Native =47配置，合计659,974配置—样本记录。新增18个在线/等范数配置，共252,756评分；原29个对应配置优先复用经身份/配方/原始分数核验的历史结果。不能只复用acc，也不把47逻辑配置称为47次新跑。旧结果不可验证时由planning允许原配方补跑，禁止换配方。正式新评分全闭合后再统一接入旧逐题outcome和gold分析；A/B只可核查旧配置/路径/schema，不能读旧逐题correctness来设计方法。

主要：K2三窗口Online−Loop和Online−Matched-norm共6项Holm family。次要K3同6项一个family；K4同6项一个family，分别报告，不替换K2主结果。Online−Shared-t0与Online−Shared-t1共18项组成一个探索性family。Native比较描述性。exact双侧McNemar，family alpha0.05；subject内配对bootstrap10000次、seed20260922，nominal95%CI。报告correct/N、micro acc、pp、错→对、对→错、净纠错、subject macro、全部负结果和资源开销。
