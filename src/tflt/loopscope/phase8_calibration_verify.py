"""Independent, outcome-free verification of saved Phase 8 calibration cells.
...
def verify_metadata(metadata, scope, pool_path, config):
    ...
    model = next((m for m in config['models'] if m['model'] == metadata['model']), None)
    ...
    _require(metadata['k'] in (2, 4) and metadata['alpha'] == 1, 'K/alpha mismatch')
...
def verify(cell_roots, pool, scope, require_all=False):
    ...
    if require_all:
        expected_cells = {(m['model'], tuple(w['layers']), k) for m in config['models']
                          for w in m['windows'] for k in (2, 4)}
        _require(seen == expected_cells, 'expected all eight canonical cells')
