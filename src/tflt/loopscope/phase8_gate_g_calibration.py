"""Gate G explicit t0 directions fitted once from unchanged K2 calibration.
...
SCHEMA = 'loopscope-phase8-gate-g-t0-basis-v1'
...
def validate_source_metadata(metadata, scope='FORMAL_CALIBRATION'):
    ...
    _require(metadata.get('model') == MODEL and metadata.get('revision') == REVISION
             and metadata.get('dtype') == 'bfloat16'
             and metadata.get('cache_strategy') == 'first'
             and metadata.get('window') == [15, 18]
             and metadata.get('alpha') == 1., 'Gate G source recipe mismatch')
