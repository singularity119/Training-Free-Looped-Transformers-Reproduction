"""Sealed acquisition and one-shot analysis for LoopScope Phase 5 Gate F.

The four variable-width cells are derived from the exact Gate E score artifact.
Before the formal structural seal closes, this module only projects canonical
sample identities and hashes evaluator payloads. Outcome parsing is confined to
``run_one_shot_analysis`` after exact 4 x 14,042 closure.
"""
...
def joint_paired_bootstrap(
    ...
    *,
    ...
    if matrix.shape != (len(subjects), 5) or len(subjects) < 2:
    ...
    estimates = np.empty((replicates, 5), dtype=np.float64)
    ...
    return {
        ...
        "same_joint_draw_stream_baseline_and_four_cells": True,
        "standard_error": "sample_sd_ddof_1",
        "percentile_method": "linear_interpolation_at_q_times_R_minus_1",
        "cell_gain_summaries": [
            _bootstrap_summary(gains[:, index].tolist()) for index in range(4)
        ],
    }
...
def build_statistical_analysis(
    *,
    ...
    matrix = np.asarray(correctness, dtype=np.int8)
    if matrix.shape != (len(identities), 5) or len(identities) != len(subjects):
        raise Phase5GateFError("analysis input shape differs")
    if len(cells) != 4:
        raise Phase5GateFError("analysis requires exactly four cells")
    ...
    return {
        ...
        "artifact_role": "one_shot_post_terminal_multiwidth_top4_outcome_analysis",
        "gate": GATE,
...
def _summary_zh(analysis: Mapping[str, Any]) -> str:
    lines = [
        "# LoopScope 第五阶段 Gate F 多宽度 Top4 全量 Outcome",
        "",
