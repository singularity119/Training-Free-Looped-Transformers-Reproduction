...
from .phase8_accuracy import select_rows
...
def verify(cell_roots, pool, manifest, full_panel=False):
    ...
    from .phase8_gate_g_accuracy import validate_manifest as validate_f
    cells = validate_f(panel, bundle, pool_path)
    ...
    for root in map(Path, cell_roots):
        ...
        scopes.add(scope)
        _require(scope == panel['scope'], 'Gate G shard/manifest scope mismatch')
        if cell['arm'] == 'Spectral':
            positions = summary['positions']
            callbacks, applied = positions['callback_by_t'], positions['applied_by_t']
            _require(set(callbacks) == set(applied) == {str(t) for t in range(cell['k'])}, 'missing timestep counts')
            _require(len(set(callbacks.values())) == 1 and callbacks['0'] > 0, 'unequal timestep callbacks')
            _require(applied['0'] == 0 and all(applied[str(t)] == callbacks[str(t)] for t in range(1, cell['k'])), 'wrong intervention timing')
